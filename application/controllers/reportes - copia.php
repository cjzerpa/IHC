	...
	class Reportes extends CI_Controller
	{
		...
		{//Controlador para manejar el inicio de la sesion
			...
			$this->load->model('Reportes_model');//llamada al modelo que hace las consultas con la BD
				$result = $this->Reportes_model->existe_usuario($_POST['nombre'] ,$_POST['pass']);//se consulta a la BD si existe el usuario
				if ($result->num_rows()>0)//funcion que de captura de erros y la query fue correcta
				{
					$row=$result->row();
			...
			/*
				$this->load->model('Reportes_model');//llamada al modelo que hace las consultas con la BD
				$result = $this->Reportes_model->existe_usuario($_POST['nombre'] ,$_POST['pass']);//se consulta a la BD si existe el usuario
				if($result !=0){	//llamada a la funcion que verifica si existe el usuario
					$newdata = array(					//creacion de las variables de sesion
							'nombre' => $_POST['nombre'],
							'pass' => $_POST['pass'],
							'logged_in' => TRUE
						);
					$this->session->set_userdata($newdata);
					if($result == '1')//verificacion si es tecnico o un usuario regular
					{
						$this->session->set_userdata('tipo_user','1');
						$this->listar();//vista para tecnico
					}
					else
					{	
						$this->session->set_userdata('tipo_user','2');
						$this->listar_reportes_usuario($this->session->userdata('nombre'));//enviar el usuario a para los reportes propios
					}				
